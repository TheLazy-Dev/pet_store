...
class SearchCubitStates {}

class SearchCubitInitial extends SearchCubitStates {}

class FoundData extends SearchCubitStates {
  final List<Pet> data;
  FoundData(this.data);
}

class NoSearchFound extends SearchCubitStates {}

class SearchCubit extends Cubit<SearchCubitStates> {
  SearchCubit() : super(SearchCubitInitial());

  final String petsData = '''
{"pets":[
  {
    "id": 10,
    "name": "Milo",
...
''';
  List<Pet> searchPetsByName(String name) {
    final List<dynamic> petsList = jsonDecode(petsData)['pets'];
    final List<Pet> matchingPets = [];

    for (final pet in petsList) {
      matchingPets.clear();
      if (pet['name'].toLowerCase().contains(name.toLowerCase())) {
        matchingPets.add(
          Pet(
            id: pet['id'],
            age: pet['age'],
